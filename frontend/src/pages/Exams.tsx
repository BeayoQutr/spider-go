...
function generateTerms(): string[] {
  ...
  // 当前学期：8月~1月为第一学期，2月~7月为第二学期
  const currentSemester = month >= 8 || month <= 1 ? 1 : 2
  const currentStartYear = currentSemester === 1 ? year : year - 1

  let startYear = currentStartYear
  let sem = currentSemester
  for (let i = 0; i < 6; i++) {
    terms.push(`${startYear}-${startYear + 1}-${sem}`)
    // 往前推一个学期
    if (sem === 1) {
      sem = 2
      startYear -= 1
...
export default function Exams() {
  const terms = useMemo(() => generateTerms(), [])
  ...
  return (
    <div className="space-y-5">
      ...
      <select
        ...
        className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500 bg-white"
      >
        {terms.map((t) => (
